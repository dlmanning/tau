...
fn generate_source(entries: &[ModelEntryData]) -> String {
    ...
    // All writeln! calls below target a String via fmt::Write, which is infallible.
    let w = "fmt::Write to String";
    writeln!(
        out,
        "// This file is auto-generated by `cargo xtask generate-models`."
    )
    .expect(w);
    writeln!(out, "// Do not edit manually.").expect(w);
    writeln!(out).expect(w);
    writeln!(out, "#[derive(Debug, Clone, Copy)]").expect(w);
    writeln!(out, "pub(crate) struct ModelEntry {{").expect(w);
    for (name, ty) in [
        ("id", "&'static str"),
        ("name", "&'static str"),
        ("provider", "&'static str"),
        ...
        writeln!(out, "    pub {name}: {ty},").expect(w);
    }
    writeln!(out, "}}").expect(w);
    writeln!(out).expect(w);
    writeln!(out, "pub(crate) const MODEL_ENTRIES: &[ModelEntry] = &[").expect(w);

    for entry in entries {
        writeln!(out, "    ModelEntry {{").expect(w);
        writeln!(out, "        id: {:?},", entry.id).expect(w);
        writeln!(out, "        name: {:?},", entry.name).expect(w);
        writeln!(out, "        provider: {:?},", entry.provider).expect(w);
        writeln!(out, "        api: {:?},", entry.api).expect(w);
        writeln!(out, "        base_url: {:?},", entry.base_url).expect(w);
        writeln!(out, "        reasoning: {},", entry.reasoning).expect(w);
        writeln!(out, "        input_text: {},", entry.input_text).expect(w);
        writeln!(out, "        input_image: {},", entry.input_image).expect(w);
        writeln!(out, "        cost_input: {},", format_f64(entry.cost_input)).expect(w);
        writeln!(
            out,
            "        cost_output: {},",
        ...
        .expect(w);
        writeln!(out, "        context_window: {},", entry.context_window).expect(w);
        writeln!(out, "        max_tokens: {},", entry.max_tokens).expect(w);
        writeln!(out, "    }},").expect(w);
    }
